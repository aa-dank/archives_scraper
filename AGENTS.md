...
- Failure handling is first-class. Failures are written to `file_content_failures`, including source provenance metadata, and successful runs clear any prior failure row. By default the worker excludes failed files from requeue; `--include-failures` retries them.
...
- Extractors inherit source metadata generation from `FileTextExtractor`; keep `system_name` as `archives_scraper`, read `system_version` from `pyproject.toml`, and use `SCRAPER_HOST_NAME` with the OS hostname as fallback for `host_name`.
- Preserve the lowercase `ocr_extraction` metadata key. It defaults to false for non-OCR extractors, is true for image extraction, and is set per call for PDF OCR fallback; OCR-aware extractors must reset their state before each invocation.
- A file is considered successfully processed only when a `FileContent` row is written. Missing extractors, extraction failures, max-char violations, and embedding failures are represented through `file_content_failures` instead of success-shaped sentinel rows. Both successful and failed rows must carry `source_metadata`.
